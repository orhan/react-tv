...
function run(root, device, install, inspect) {
  const totalSteps = install ? (inspect ? 5 : 4) : 3;
  ...
  if (!device) {
    console.log('');
    console.log(chalk.red('❗️  You haven\'t specified a device, listing your webOS devices:'));
    console.log('');
  ...
  const task = setInterval(function() {
    if (!device) {
      ...
      console.log('');
      console.log(chalk.dim(`[1/${totalSteps}] `) + chalk.yellow((inspect ? '🔧  Preparing to debug' : '👟  Preparing to run') + ` "${config.title}" on "${device}"...`));
    }
    ...
    if (install) {
      ...
      if (inspect) {
        console.log('');
        console.log(chalk.dim(`[5/${totalSteps}] `) + chalk.yellow('🔬  Inspecting app...'));
        console.log('');
      ...
      console.log(chalk.dim(`[3/${totalSteps}] `) + chalk.dim(`✨  Successfully built release package: `) + chalk.blue(`${webosPath}/${latestIPK}`));
    }
